...
bestplastrees=bestplastrees[1:10000]
bestplastrees=root(bestplastrees,"GALLUS06",resolve.root = TRUE)
#https://intellipaat.com/community/16277/how-to-overlay-density-plots-in-r density plots for phylogenetic parameters
chunk_length=100
list=sample(10000,replace=F)
plaslist=split(list,             # Applying split() function
      ceiling(seq_along(list) / chunk_length))
plasarboles<-list()
maxplasarboles<-list()

for (i in 1:100){
  plasarboles[[i]]<-bestplastrees[c(plaslist[[i]])]
  maxplasarboles[[i]]<-maxCladeCred(plasarboles[[i]],rooted=T)
  }
class(maxplasarboles)<-"multiPhylo"
matreesDIstplasprob <- treespace(maxplasarboles,nf=2)
...
xplas <- bwplas$Axis1
yplas <- bwplas$Axis2
plasinner <- point.in.polygon(bwplas$Axis1, bwplas$Axis2, ls[[1]]$x, ls[[1]]$y)
table(plasinner)
plasout <- point.in.polygon(bwplas$Axis1, bwplas$Axis2, ls[[2]]$x, ls[[2]]$y)
table(plasout)
## Plot
bwplas$region <- (plasinner + plasout)
bwplas["region"][bwplas["region"] >0]  <- 1
bwplas$region <-factor(bwplas$region)
plot(Axis2 ~ Axis1, col=region, data=bwplas[c(1:2500),], pch=15)
contour(dens, add=T)
#FIRST SAMPLE OF TREES
firstsetplas=which(bwplas$region[1:2500]==1,T)
plassampdef=sample(firstsetplas,1000)
write.tree(plastrees[plassampdef],"plassampletreesmax")
...
besthaetrees=besthaetrees[1:10000]
besthaetrees=root(besthaetrees,"GALLUS06",resolve.root = TRUE)
#https://intellipaat.com/community/16277/how-to-overlay-density-plots-in-r density plots for phylogenetic parameters
chunk_length=100
list=sample(10000,replace=F)
haelist=split(list,             # Applying split() function
               ceiling(seq_along(list) / chunk_length))
haearboles<-list()
maxhaearboles<-list()

for (i in 1:100){
  haearboles[[i]]<-besthaetrees[c(haelist[[i]])]
  maxhaearboles[[i]]<-maxCladeCred(haearboles[[i]],rooted=T)
}
class(maxhaearboles)<-"multiPhylo"
matreesDIsthaeprob <- treespace(maxhaearboles,nf=2)
