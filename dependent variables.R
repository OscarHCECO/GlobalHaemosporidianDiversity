...
plaspsv1000df=plaspsv1000%>%cbind(Pid)%>%merge(plasgeo,by="X")
View(plaspsv1000df)
...
haepsv1000df=haepsv1000%>%cbind(Hid)%>%merge(haegeo,by="X")
View(haepsv1000df)
write.csv(haepsv1000df,"haepsv1000.csv")#Save PSV for Haemoproteus assemblages
leunewdata=foreach::foreach(i = 1:10)%dopar%{#Leucocytozoon data
...
leupsv1000df=leupsv1000%>%cbind(Lid)%>%merge(leugeo,by="X")
View(leupsv1000df)
...
plasrichness=plaspresabF[-1]%>%rowSums()%>%as.data.frame()%>%cbind(Pid)#SR for Plasmodium
names(plasrichness)=c("plasrichness","X")
plaspd1000=do.call(cbind, plaspddef)
plaspd1000=plaspd1000%>%cbind(Pid)%>%na.omit()
dim(plaspd1000)
dataplas=list()
plasrpd=list()
for (i in 1:10){
  dataplas[[i]]=plaspd1000[c(i,11)]%>%merge(plasrichness,by="X")
...
plasrpd1000df=plasrpd1000%>%cbind(plaspd1000[11])%>%merge(plasgeo,by="X")
names(plasrpd1000df)=c("X",rep("rPD",10),"poligons","x","y")
View(plasrpd1000df)
write.csv(plasrpd1000df,"plasRPD1000.csv")#Save RPD for Plasmodium assemblages
haepd=foreach::foreach(i = 1:10)%dopar%{
...
haerichness=haepresabF[-1]%>%rowSums()%>%as.data.frame()%>%cbind(Hid)
names(haerichness)=c("haerichness","X")
haepd1000=do.call(cbind, haepddef)
haepd1000=haepd1000%>%cbind(Hid)%>%na.omit()
dim(haepd1000)
##rpd
datahae=list()
haerpd=list()
for (i in 1:10){
  datahae[[i]]=haepd1000[c(i,11)]%>%merge(haerichness,by="X")
...
haepsv1000df=haerpd1000%>%cbind(haepd1000[11])%>%merge(haegeo,by="X")
names(haepsv1000df)=c("X",rep("rPD",10),"poligons","x","y")
View(haepsv1000df)
