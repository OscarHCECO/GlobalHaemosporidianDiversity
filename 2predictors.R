...
library(rgdal)
library(raster)
birdtree<-read.tree("MXCLADECREDBIRDTREE.tre")#maximum credibility tree of birds from 100 jetz phylogenetic trees
plaspresab<-read.csv("plasmodiumPAM")#Load -parasite lineages x locality- matrices
haepresab<-read.csv("haemoproteusPAM")
leupresab<-read.csv("leucocytozoonPAM")
hostxlineages<-read.csv("hostxlineages.csv")#matrix of parasite lineages x host species 
...
malavidataframe<-read.csv("malavisubset.csv")#dataframe with a subset of lineages records from MalAvi (coordinates and genus)
regiongrid<-readOGR("regionsgrid.shp")#Load a grid of the studied regions (0.25°x0.25°resolution)  Í
